...
public class MazeService {
    ...
    public void generateRecursiveDivisionMaze(Tile[][] grid, Position start, Position end) {
    ...
    // Ensure start and end tiles are not walls
    grid[start.getRow()][start.getCol()].setWall(false);
    grid[end.getRow()][end.getCol()].setWall(false);

    divide(grid, 0, 0, cols, rows, start, end);
}

private void divide(Tile[][] grid, int x, int y, int width, int height, Position start, Position end) {
    if (width <= 2 || height <= 2) {
        return;
    }
    ...
    for (int i = 0; i < length; i++) {
        ...
        // Skip if the tile is the passage point or the start/end tile
        if ((nx == px && ny == py) || (nx == start.getCol() && ny == start.getRow()) || (nx == end.getCol() && ny == end.getRow())) {
            continue;
        }
    ...
    divide(grid, nx, ny, w, h, start, end);
